...
(define (get-indexed-assertion pattern)
...
(define (add-assertion! assertion)
  ...
  (let ((old-assertions THE-ASSERTIONS))
    (set! THE-ASSERTIONS
	  (cons-strem assertion old-assertions))
    `ok))
  ...
  (let ((pattern (conclusion rule)))
	...
	(let ((key (index-key-of pattern)))
	  (let ((current-rule-stream (get-stream key `rule-stream)))
	    (put key `rule-stream
		 (cons-stream rule current-rule-stre)))))))
...
(define (stream-append-delay s1 delayed-s2)
  (if (stream-null? s1)
      ...
      (cons-stream (stream-car s1)
		   (stream-append-delay (stream-cdr s1) delayed-s2))))

(define (interleave-delay s1 delayed-s2)
  (if (stream-null? s1)
      (force delayed-s2)
      (cons-stream (stream-car s1)
		   (interleave-delay (force delayed-s2)
				     (delay (stream-cdr s1))))))

(define (stream-flatmap proc seq)
  (flatten-stream (stream-map proc seq)))
(define (flatten-stream seq)
  (if (stream-null? seq)
      the-empty-stream
      (interleave-delayed (stream-car seq)
			  (delay (flatten-stream (stream-cdr seq))))))
